...
class HotspotprofileAPI(MethodResource,  Resource):
    ...
    def post(self, **kwargs):
        try:
            ...
            new_log = hotspotprofile_logging_create(accessed, str(data), data['id'], info_admin)
            if new_log == False:
    ...
    def put(self, **kwargs):
        try:
            ...
            if get_profile:
                ...
                info_admin = info_administrator()
                accessed = {'ip':request.remote_addr, 'id_token': info_admin['request_id']}
                new_log = hotspotprofile_logging_update(accessed, str(data), data['id'], info_admin)
                if new_log == False:
                    print("Logging Failed")
    ...
    def delete(self, **kwargs):
        try:
            ...
            if get_profile:
                ...
                info_admin = info_administrator()
                accessed = {'ip':request.remote_addr, 'id_token': info_admin['request_id']}
                new_log = hotspotprofile_logging_delete(accessed, str(old_data), old_data['id'], info_admin)
                if new_log == False:
                    print("Logging Failed")
