...
def plandefault_logging_create(accessed, planttypecreate, refrence_id, administrator_id):
    new_log = admin_log(accessed, 'plan_default', 'create', planttypecreate, refrence_id, administrator_id)
    db_admin.session.add(new_log)
    db_admin.session.commit()
    return True

def plandefault_logging_update(accessed, planttypecreate, refrence_id, administrator_id):
    new_log = admin_log(accessed, 'plan_default', 'update', planttypecreate, refrence_id, administrator_id)
    db_admin.session.add(new_log)
    db_admin.session.commit()
    return True

def plandefault_logging_delete(accessed, planttypecreate, refrence_id, administrator_id):
